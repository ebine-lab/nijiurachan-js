  ...
  function UpfileInputFragmentV2(props: UpfileInputV2Props): VNode {
    ...
    useLayoutEffect(() => {
      props.bindCommands?.({
        ...
        clickPaint: () => dispatchRef.current(selectedPaintAction(paintPopups)),
        clickKlecks: () => {
    ...
    return (
      <>
        ...
        >
          ...
          {!controls.oejsCanvas && controls.previewFigure && previewBlob && (
            <PreviewMedia file={previewBlob} />
          )}
        </figure>
      </>
    )
    ...
    function canOpenPaint(tool: OekakiTool, action: UpfileAction): boolean {
      ...
      if (!isPaintPopupAvailable(paintPopups, tool)) {
        console.warn(`[upfile-input-v2] ${tool} popup is not configured`)
        return false
      }
      return true
    }
