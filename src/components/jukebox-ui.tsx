...
export function JukeboxUI(props: JukeboxUIProps): VNode {
	const { state, onEnqueue, onSkipVote, onCancelMine, enqueueError } = props;
	...
	return (
		<div class="jukebox">
				...
				{state?.nowPlaying ? (
					<>
						...
						{state.nowPlaying.isReplay && (
							<span class="jukebox-replay-badge" aria-label="ラジオ自動再生">
								♻️ ラジオ（自動再生）
							</span>
						)}
						<button
						...
						>
							{state.mySkipVoted ? "スキップ投票済み" : "スキップ投票"}
						</button>
					</>
				) : (
					<span>再生なし</span>
			...
			<div id="jukebox-yt-player" />

			<ul class="jukebox-queue">
				{state?.queue.map((item: JukeboxQueueItem) => (
					<li key={`${item.source}:${item.mediaId}`}>
			...
			<form class="jukebox-enqueue-form" onSubmit={(e) => void handleEnqueue(e)}>
				<input
					type="url"
					value={urlInput}
				...
				/>
				<button type="submit" disabled={submitting || onCooldown || !urlInput.trim()}>
