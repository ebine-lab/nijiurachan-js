...
export function JukeboxUI(props: JukeboxUIProps): VNode {
  ...
  const queueEtas =
    state != null
      ? computeQueueEtaMs(state.nowPlaying, state.queue, state.serverNowMs)
      : []
    ...
    <div class="jukebox">
        ...
        {state?.queue.map((item: JukeboxQueueItem, index: number) => {
          ...
          return (
            <li key={item.id}>
              ...
              />
              {item.mine && (
                <CancelButton trackId={item.id} onCancelMine={onCancelMine} />
              )}
            </li>
          )
        })}
      ...
      {showHistory && (
        <ul class="jukebox-history">
          {history.length === 0 ? (
            <li class="jukebox-history-empty">まだ履歴がありません</li>
          ) : (
