...
export class KlecksPaintHostElement extends HTMLElement {
    ...
    klecks = new Klecks({
      ...
      onSubmit: async (
        ...
        try {
          ...
          const cloudDraft = await this.#saveCloudDraft(klecks, image).catch(
            (error: unknown) => {
  ...
  async #latestCloudDraft(
    ...
    if (!response.ok || !result.ok) {
      throw new Error(
        result.ok ? "Klecks cloud draft index load failed" : result.error,
      )
    }
  ...
  #send(image: Blob | null, cloudDraft: CloudDraftResult | null = null): void {
...
function readCloudDraftState(): CloudDraftState {
  try {
    ...
    return {
      saveKey: typeof parsed.saveKey === "string" ? parsed.saveKey : undefined,
      drafts:
