    ...
    if (this.#currentTrackId === np.id && this.#ytPlayer) {
      ...
      const localPositionSec =
        typeof this.#ytPlayer?.getCurrentTime === "function"
          ...
          : 0
      if (Math.abs(localPositionSec - expectedSec) > DRIFT_THRESHOLD_SEC) {
        ...
        events: {
          onReady: (e: YTPlayerEvent): void => {
            ...
            const readyProjectedMs =
              state.serverNowMs + (Date.now() - this.#fetchedAtClientMs)
  ...
  #renderUI(): void {
    render(
      h(JukeboxUI, {
        ...
        onCancelMine: (trackId: number) => this.#handleCancelMine(trackId),
